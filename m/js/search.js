...
$(function() {
  ...
  var SETTINGS = {
    AIR: {
      ...
      RESERVATION: location.protocol + '//' + location.hostname + '/sp/air/request'
    },
    HOTEL: {
      HISTORY: {
        SEARCH: 5,
      ...
      RESERVATION: location.protocol + '//' + location.hostname + '/sp/hotel/request'
    },
    TOUR: {
        HISTORY: {
          SEARCH: 5,
        ...
        RESERVATION: location.protocol + '//' + location.hostname + '/sp/tour/request'
    },
    PAYMETHOD: {
  ...
  var Func = {
    ...
    url: function(path, query, callback) {
      ...
      return Status.basepath + path.replace(/^\//, '') + append;
    },
    ...
    amoundetail: function(air, htl) {
      ...
      if (air.point | 0 > 0) {
        var amountpoint = ~~air.point + ~~htl.chargeablerateinfo.point;
        $('.amount.dp-amount .amount__point').find('.point').text(amountpoint.toString().replace(/(\d)(?=(\d{3})+$)/g, '$1,')).end().show();
      } else {
      ...
      var htlPaymentTotal = parseInt(htl.chargeablerateinfo.paymentTotal);
      var htlDiscount = parseInt(htl.chargeablerateinfo.discount);
      var htlPkg = parseInt(htl.chargeablerateinfo.pkgdiscount);
      if (htl.chargeablerateinfo.discount) {
        htlprice = htlPaymentTotal + htlDiscount + htlPkg;
        var discount = htlDiscount + htlPkg;
    ...
    clear: function () {
      ...
      Func.history('tour/cond', false);
      Func.history('air/cond', false);
      Func.history('hotel/cond', false);
      Func.history('air/search/departure', false);
      Func.history('hotel/search', false);
      Func.history('metasearch/param', false);
    },
    ...
    paramcheck: function(type) {
      if (type === 'air' || type === 'tour') {
        var required = [
          'traveler',
          'flight.1',
        ];
      ...
      if (type === 'air' && conditions.referer !== 'oneway' ) {
        required.push('flight.2');
      }
      var i = 0;
      for (i in required) {
        if (!conditions[required[i]]) {
          flg = 0;
          break;
        }
      }
      if (
        type === 'hotel' &&
        ((!conditions['hotel'] && !conditions['region']) ||
        !conditions['stay'].match(/^\d{8}-\d{8}$/))
      ) {
        flg = 0;
      }
      return flg;
    },
    metavalidity: function (path) {
      var flg = true;
      if (path[0] === 'air') {  
        if (path[1] !== 'search') {
          flg = false;
    ...
    metafrom: function() {
      ...
      var nowArr = now.replace(/^https?:\/\//, '').split(/\//);
      var refArr = ref.replace(/^https?:\/\//, '').split(/\//);
      if (Func.metaitemcodecheck() && (nowArr[0] != refArr[0] || nowArr[1] != refArr[1])) {
        return true;
      }
      if ((typeof document.referrer === 'string' && document.referrer.length === 0) || (Func.getdomain(now) != Func.getdomain(ref)) || (nowArr[0] != refArr[0] || nowArr[1] != refArr[1])) {
        flg = true;
      }
      return flg;
    },
    ...
    normalparam: function() {
      var metaParams = Func.metaparampullout();
      var params = {};
      var i = 0;
      for (i in Status.search) {
        if (!metaParams[i]){
          params[i] = Status.search[i];
        }
      }
      return params;
    },
    metaitemcodecheck: function() {
      if (Status.pathname[0] == 'air' || (Status.pathname[0] == 'tour' && Status.pathname[1] === 'search')) {
        var required = [
          'coursecd',
          'goods_code',
          'airno',
        ];
      } else if (Status.pathname[0] === 'hotel' || (Status.pathname[0] == 'tour' && Status.pathname[1] === 'hotel')) {
        var required = [
          'hotel',
        ];
      }
    ...
    metaparampullout: function() {
      ...
      var i = 0;
      for (i in Status.search) {
        if (jQuery.inArray(i, trueParam) === -1 && !i.match(/^flight\./) ){
          pullData[i] = Status.search[i];
        }
      }
      return pullData;
    },
    firstparamsave: function(){
      Func.history('metasearch/param', Status.search);
    },
    firstparamcheck: function(build){
      var firstCondition = Func.history('metasearch/param').shift();
      if (firstCondition == undefined) {
        return false;
      }
      var _build = $.extend(true, {}, build);
      var metaParam =  Func.metaparampullout();
      $.each(metaParam, function (key) {
        delete _build[key];
        delete firstCondition[key];
      });
      var flg = true;
      $.each(_build, function (key) {
        if (typeof firstCondition[key] == "undefined" || firstCondition[key] != this) {
          flg = false;
          return true;
        }
      });
    ...
    firstparamchange: function (build) {
      var type = 'air';
      if (jQuery.inArray('hotel', Object.keys(Status.search) ) >= 0 && jQuery.inArray('source', Object.keys(Status.search) ) >= 0 ){
        type = 'hotel';
      }
      var _build = $.extend(true, {}, build);
      var param = Func.normalparam();
      var metaParam = Func.metaparampullout();
      var keyList = Func.parameterlist(type);
      var diffParam = Func.paramfilter(param, keyList);
      var diffBuild = Func.paramfilter(_build, keyList);
      var changeFlg = false;
      $.each(diffParam, function (index) {
        if (this != diffBuild[index]) {
          changeFlg = true;
          return true;
        }
      });
      if (changeFlg === true) {
        $.each(metaParam, function (index) {
          delete build[index];
        });
      }
    },
    parameterlist: function (type) {
      if (type == 'air') {
        var param = [
          'seat',
          'airline',
        ...
        var objKeys = Object.keys(Status.search);
        if (!jQuery.inArray('hotel', objKeys ) && jQuery.inArray('region', objKeys )) {
          param.push('region');
        }
      }
      return param;
    },
    paramfilter: function (param, filter) {
      var data = {};
      $.each(filter, function (index) {
        data[this] = null;
        if (param[this]) {
          data[this] = param[this];
        }
      });
      return data;
    }
  ...
  var Favorite = (function() {
    ...
    methods.updateStatus = function() {
      var header = $('#js-show-header-menu');
      var contents = $('#js-header-menu .menu-list .menu-list__section:first .js-accordion-contents');
      var loader = function(target) {
        return $.ajax({
          method: 'POST',
          url: Func.url(target + '/data/favorite/exists', Func.storage(['session'])),
          contentType: 'application/json',
          dataType: 'json'
        });
      };
      var update = function(result, n) {
        var menu = contents.find('a:eq(' + n + ') .menu-list-item__text').removeClass('is-caution');
        if (result.exists) {
          menu.addClass('is-caution');
          header.addClass('is-alert');
        }
      };
      $.when(loader('air'), loader('hotel')).done(function(r1, r2) {
        header.removeClass('is-alert');
        update(r1.shift(), 0);
        update(r2.shift(), 1);
      });
    };

    methods.bindSwitch = function(type) {
      $('.js-toggle-fav').on('click', function() {
        var entry = $('#js-modal-detail').data('entry');
        var urls = [
          type,
          'data/favorite',
          $(this).hasClass('is-fav') ? 'register' : 'remove'
        ];
        var data = {id: type === 'hotel' ? entry.hotelid : entry.id};
        var option = {
          method: 'POST',
          url: Func.url(urls.join('/'), Func.storage(['session'])),
          contentType: 'application/json',
          data: JSON.stringify(data),
          dataType: 'json'
        };
        $.ajax(option).done(function(loaded) {
    ...
    var Entry = {
      ...
      view: function(entries, key) {
        ...
        var embed = function(item, data) {
          ...
          if (data.point|0 > 0) {
            item.find('.data-point__num').text(data.point);
          } else {
            item.find('.data-point').text('').hide();
          }
          ...
          if ((!traveler.adult || !traveler.child || traveler.adult === traveler.child) || data.itemtype === 'LCC' ) {
          ...
          if (data.itemtype === 'LCC') {
            item.find('.js-feature-tooltip li:eq(2)').hide();
            item.find('.js-feature-tooltip li:eq(3)').hide();
          } else if (data.ConditionsOfUse.ticketType === 'IT') {
            item.find('.js-feature-tooltip li:eq(3)').hide();
            item.find('.js-feature-tooltip li:eq(8)').hide();
          ...
          item.find('a.js-show-modal').on('click', function() {
            ...
            if (!format) {
              ...
              format.lcc = modal.find('.lcc-system-usage-fee').detach();
            ...
            for (var i = 0; i <= last; i ++) {
              ...
              for (var j = 0; j < f.Segments.length; j ++) {
                ...
                if (s.Carrier.classOfService.bookingclass) {
                  carrier.push('(' + s.Carrier.classOfService.bookingclass + ')');
                }
            ...
            $.each(traveler, function() {
              var n = this;
              var taxHideFlg = false;
              $.each(data.TravelerGroups, function() {
                if (data.itemtype === 'LCC' && this.Price.Taxes.total == 0 && this.type == 'ADT') {
                  taxHideFlg = true;
                }
                if (this.type === n.type) {
            ...
            if (data.itemtype === 'LCC') {
              var lcc = format.lcc.clone(true);
              lcc.find('dl:eq(0) dd').text(Func.money(data.systemUsageFee, 'suffix')).end();
              modal.find('.price-detail').append(lcc);
            }
            ...
            modal.find('.selectairplan').off('click').on('click', function() {
              ...
              //Func.history('air/detail', JSON.stringify({key: key, id:data.id, arrival:arrival.arrival, paymentmethods:data.ConditionsOfUse.PaymentMethods, price: airprice, time:new Date().getTime()}));
              // モーダル閉じる
              modal.find('.js-close-modal').trigger('click');
              location.reload();
            });
            if (data.itemtype === 'LCC') {
              modal.find('.button-area a.is-set').hide();
            } else {
              modal.find('.button-area a.is-set').show();
            }
...
//              }
              ...
              $.ajax(option).done(function(result) {
                ...
                //Func.history('air/detail', JSON.stringify({key:key_, id:data.id, arrival:arrival.arrival, paymentmethods:data.ConditionsOfUse.PaymentMethods, price: airprice, time:new Date().getTime()}));
                // DPの航空券検索結果画面へ遷移
              ...
              .on('click', function() {
                ...
                $.ajax(option).done(function(result) {
                  if (result.status === 'success') {
                    ...
                    var defaultReferrer = location.href;
                    var param = {
                      ...
                      referer: defaultReferrer,
                      sessionkey: result.sessionkey,
                      url: document.referrer || defaultReferrer,
                      route: routes.join('|'),
                      backparam: 'spback=true',
                    };

                    if (Status.search.source && Status.search.source.length > 0) {
                      param.referer = Status.search.source;
                    }
                    $('<form/>')
                      .attr({
                        method: 'POST',
        ...
        var selectedAirid = null;
        var create = function() { 
          if (entries.length > created.length) {
            var pager = SETTINGS.AIR.PAGER;
            var airDetail = Func.history('air/detail').shift();
            if (airDetail && Status.pathname[0] === 'tour' ) {
              airDetail = JSON.parse(airDetail);
              selectedAirid = airDetail.id;
            }
            $.each(entries, function() {
              if (!this._hidden && created.indexOf(this.id) < 0 && this.id != selectedAirid) {
                if (-- pager >= 0) {
                  if (!this.selected && this.id != selectedAirid) {
              ...
              if (!metaSelected && (this.id == selectedAirid || this.selected) ) {
                if (selectedAirid) {
                  if (this.id == selectedAirid) {
                    var cItemObj = itemObj.clone(true);
                    metaSelected = 'Y';
                    $('#selected-info').hide();
                    list.prepend(embed(cItemObj, this));
            ...
            if (Func.metafrom() && !metaSelected) {
              $('#selected-info .selected-text').html('<span class="selected-notfind">大変申し訳ございません。</span>お選びいただいた日程で、お探しの条件に合う利用可能なチケットが見つかりませんでした。');
            }
          }
        };
        create();
    ...
    methods.bindAirport = function() {
      ...
      var initialize = function(count) {
        ...
        if (brandData && brandData.domain == 'platinum-travel-air.jp') {
          path.push(count);
        }
        var click = function() {
          var id = $(this).data('id');
          ...
          .on('keyup touchend change', function(event) {
            ...
            if (value !== '') {
              $.each(location_index, function(index) {
                ...
                if (brandData && brandData.domain == 'platinum-travel-air.jp') {
                  if ((e.countrycode !== 'JP' && $.inArray('departure', path) > 0 && count === 0) ||
                    (e.countrycode === 'JP' && $.inArray('departure', path) > 0 && count === 2) || 
                    (e.countrycode === 'JP' && $.inArray('arrival', path) > 0 && count === 1) ||
                    (e.countrycode !== 'JP' && $.inArray('arrival', path) > 0 && count === 3)) {
                    return true;
                  }
                }
    ...
    methods.bindSearch = function(type, locationurl) {
      ...
      $(targets.join(',')).bind('click', function() {
        ...
        var brandData = Func.history('top/brand').shift();
        if ((!brandData || brandData.domain == 'ena.travel') && traveler.adult + traveler.child + traveler.baby > 9) {
          return Func.error('*SEARCH_TOO_MANY_TRAVELERS');
        }

        if (brandData && traveler.adult + traveler.child + traveler.baby > 11) {
          return Func.error('*SEARCH_TOO_MANY_TRAVELERS_11');
        }
        ...
        var conditions = {
          referer: Status.search.referer || type,
        ...
        if (Func.metafrom()) {
          $.extend(conditions, Func.metaparampullout());
        }
        ...
        if (historyPath == 'tour/cond') {
          ...
          // DPの場合はホテルのリージョン情報をまず手に入れる
          /*var e = getLocation(arrivalID);
          if (typeof e != 'object'){
            return Func.error('*SEARCH_MISSING_STAYPLACE');
          }
          var arrivalName = e.jp || e.cityname || e.airport;
          if (-1 < arrivalName.indexOf(',')){
            arrivalName = e.cityname || e.airport;
          }
          if (0 >= arrivalName.length){
            return Func.error('*SEARCH_MISSING_STAYPLACE');
          }*/
          ...
          $.ajax(option).done(function(result) {
            ...
            for (var didx=0; didx < result.found.length; didx++){
              if (arrivalID == result.found[didx].fid){
                ...
                if (true != (('region' in conditions) || ('hotel' in conditions))){
                  conditions[data.datatype] = data.id;
                }
                else if (0 > location.pathname.indexOf('tour/hotel')){
                  if (true == ('hotel' in conditions) && data.datatype != 'hotel'){
                    conditions[data.datatype] = data.id;
                  }
                  else if (true == ('region' in conditions) && data.datatype != 'region'){
                    conditions[data.datatype] = data.id;
                    delete conditions.region;
                  }
                }
            ...
            $.extend(conditions, Func.metaparampullout());
            if (Func.metafrom() && Status.pathname[0] === 'air'){
              conditions['region'] = result['found'][0]['id'];
            }
            location.href = Func.url(locationurl, conditions, function(build) {
              // TOPからDPへの遷移
              if (historyPath == 'tour/cond' && 0 > location.pathname.indexOf('/m/tour/search') && -1 < location.pathname.indexOf('/m/tour') && 0 > location.pathname.indexOf('/air') && 0 > location.pathname.indexOf('/hotel')) {
                // キャッシュをクリアする
                Func.history('tour/detail/airdom', false);
                Func.history('tour/detail/hoteldom', false);
                Func.history('air/detail', false);
              ...
              if (historyPath == 'tour/cond' && Status.pathname[0] === 'air') {
                Func.history('hotel/detail', false);
                Func.history('tour/detail/hoteldom', false);
              }
              if (historyPath == 'tour/cond' && document.referrer.match(/m\/top$/)) {
                Func.history(historyPath+'/cache', 'clear');
              }
              var _condition = Func.history(historyPath).shift();
              if (_condition != undefined){
                // 検索条件のマージ
                if (true == ('region' in _condition)){
                  build.region = _condition.region;
                }
                if (true == ('hotel' in _condition)){
                  build.hotel = _condition.hotel;
                }
                if (true == ('room' in _condition) && true != ('room' in build)){
                  build.room = _condition.room;
                ...
                // 航空券選択キャッシュ削除判定
                if (true == ('airline' in build) && false == (true == ('airline' in _condition) && true == ('airline' in build) && build['airline'] == _condition['airline']) ) {
                  Func.history('tour/detail/airdom', false);
                  Func.history('air/detail', false);
                }
                // 航空券選択キャッシュ削除判定
                ...
                // ホテル選択キャッシュ削除判定
                if (true == ('region' in build) && false == (true == ('region' in _condition) && build['region'] == _condition['region'])
                  //|| true == (true == ('hotel' in _condition) && true == ('hotel' in build) && build['hotel'] == _condition['hotel'])
                ) {
                  Func.history('tour/detail/hoteldom', false);
                  Func.history('hotel/detail', false);
              ...
              if (historyPath == 'tour/cond' && true != ('room' in build)){
                // room保管
                var rooms = [];
                $('#js-modal-people .js-people-selector').each(function() {
                  var adult = $(this).find('.js-people-num-adult').val()|0;
                ...
                if (rooms.length > 0) {
                  build['room'] = qRoom.encode(rooms);
                }
              }
              if (Func.firstparamcheck(build) == false) {
                Func.history('metasearch/paramchange', 'on');
              }
              Func.firstparamchange(build);
        ...
        location.href = Func.url(locationurl, conditions, function(build) {
          ...
          var brand = Func.history('top/brand').shift();
          if (brand && brand.domain == 'platinum-travel-air.jp') {
            build.referer = 'multi-city';
          }
          Func.firstparamchange(build);
          Func.history(historyPath, build);
        });
    ...
    methods.restoreConditions = function(type, targetpath) {
      if (typeof targetpath == 'undefined'){
        targetpath = 'air';
      }
      var condition = Func.history(targetpath+'/cond').shift();
      if (condition === undefined) {
        return;
      }

      var form = function(target, flight) {
        target.find('.is-departure')
          .find('.search-item__title').addClass('is-left is-half')
          .after($('<span>').addClass('search-item__value').text(flight.departure));
        target.find('.is-arrival')
          .find('.search-item__title').addClass('is-left is-half')
          .after($('<span>').addClass('search-item__value').text(flight.arrival));
        target.find('.flight-nonstop').prop('checked', flight.direct);
        if (undefined == flight.departure || '' == flight.departure || null == flight.departure){
          $('.search-error p').html(SETTINGS.ERROR.SEARCH_MISSING_DEPARTURE);
          $('.search-error').show();
        }
      };
      if ($('.js-form-flight').length > 0) {
        var exists = 0;
        for (var i = 1; i <= 6; i ++) {
          var name = Func.vsprintf('flight.%d', [i]);
          if (condition[name] && $('.js-form-flight').length < ++ exists) {
            $('#js-add-flight').trigger('click');
          }
        }
        $('.js-form-flight').each(function(index) {
          var name = Func.vsprintf('flight.%d', [index + 1]);
          var flight = qFlight.decode(condition[name]);
          if (flight) {
            form($(this), flight);
            Calendar.setValue(index, {from: flight.date});
          }
        });
      } else {
        var departure = qFlight.decode(condition['flight.1']);
        if (departure) {
          form($('.search-form'), departure);
          var range = {from: departure.date};
          var arrival = qFlight.decode(condition['flight.2']);
          if (arrival && departure.departure === arrival.arrival && departure.arrival === arrival.departure) {
            range.to = arrival.date;
          }
          switch (type) {
            case undefined:
            case 'abroad':
              if (!range.to) {
                range = Calendar.defaultValue(range);
              }
              break;
            case 'oneway':
              delete range.to;
          ...
          Calendar.setValue(0, range);
        }
      }

      if ('tour' != Status.pathname[0]){
        if (condition.traveler) {
          var traveler = qTraveler.decode(condition.traveler);
        }
        if (traveler && traveler.adult > 0) {
          var modal = $('#js-modal-people')
            .find('.js-people-num-adult').val(traveler.adult).trigger('change').end()
            .find('.js-view-age').val(0).trigger('change').end();
          $.each(traveler.children, function(index) {
            modal.find('.js-view-age').siblings('.is-plus').trigger('click').end();
            var item = modal.find('.js-age-item').eq(index);
            item.find('.js-select-child-age').val([this.age]).trigger('change').end();
            if (item.find('.js-switch-seat').hasClass('is-checked-disable')) {
              item.find('.js-switch-seat').prop('checked', true);
            } else {
              item.find('.js-switch-seat').prop('checked', this.seat);
            }
            setTimeout(function() {
              item.find('.js-switch-seat').trigger('change').end();
            }, 50);
          });
        }
      }

      if (condition.airline || condition.seat) {
        $('.search-option').addClass('is-open')
          .find('> a').addClass('is-open').siblings().show();
      }
      if (condition.airline) {
        $('#js-modal-airline a').each(function() {
          $(this).data('id') === condition.airline && $(this).trigger('click');
        });
      }
      if (condition.seat) {
        $('#js-modal-seat a').each(function() {
          $(this).data('id') === condition.seat && $(this).trigger('click');
        });
      }
      if (condition.stay && 0 < $('.search-form__row.hoteloptioncalendar #part-trip').size()) {
        var range = {from:false, to: false};
        var _departure = qFlight.decode(condition['flight.1']);
        if (departure) {
          range.from = departure.date;
        }
        var _arrival = qFlight.decode(condition['flight.2']);
        if (_arrival && _departure.departure === _arrival.arrival && _departure.arrival === _arrival.departure) {
            range.to = _arrival.date;
        }
        var airrange = qStay.encode(range);
        if (condition.stay != airrange){
          $('.search-form__row.hoteloptioncalendar #part-trip').prop('checked', true);
          $('.search-form__row.hoteloptioncalendar .js-accordion-wrapper').addClass('is-open');
          $('.search-form__row.hoteloptioncalendar .js-accordion-wrapper .js-toggle-accordion').addClass('is-open');
          $('.search-form__row.hoteloptioncalendar .js-accordion-wrapper .js-accordion-contents').show();
          Calendar.setValue(1, qStay.decode(condition.stay));
        }
        else {
          $('.search-form__row.hoteloptioncalendar #part-trip').prop('checked', false);
        }
      ...
      /*if (condition.room) {
          var modal = $('#js-modal-people');
          modal.find('.js-step-rooms').val(0).trigger('click').val(1).trigger('click');
          $.each(qRoom.decode(condition.room), function(index) {
            index > 0 && modal.find('.js-step-rooms').siblings('.is-plus').trigger('click');
            var block = modal.find('.js-people-selector').eq(index);
            var dist = function(self, n) {
              var d = n - self.val()|0;
              if (d !== 0) {
                var t = d > 0 ? '.is-plus' : '.is-minus';
                for (var i = Math.abs(d); i > 0; i --) {
                  self.siblings(t).trigger('click');
                }
              } else {
                self.trigger('change');
              }
            };
            dist(block.find('.js-people-num-adult'), this.adult);
            dist(block.find('.js-view-age'), this.children.length);
            $.each(this.children, function(index) {
              block.find('.js-age-item').eq(index).find('select').val([this]);
            });
          });
        }*/
    };
    ...
    methods.embedConditions = function() {
      ...
      if (Status.search.airline != undefined) {
        if (AIR_LINE[Status.search.airline]) {
          $('[data-target="js-airline-value"] .open-selector__item.js-select-item.is-active').removeClass('is-active');
          panel.find('.airline .search-item__data').text(AIR_LINE[Status.search.airline]);
          var airline = $('[data-target="js-airline-value"] a.open-selector__item');
          for (var i=0; i < airline.length; i++){
            if (AIR_LINE[Status.search.airline] == airline[i].innerText) {
              airline[i].className = 'open-selector__item js-select-item is-active';
              break;
            }
          }
        }
      ...
      if (Status.search.seat != undefined) {
        var classes = {
          'EC': 'エコノミー',
          'PE': 'プレミアムエコノミー',
          'BU': 'ビジネス',
        ...
        if (classes[Status.search.seat]) {
          $('[data-target="js-seat-value"] .open-selector__item.js-select-item.is-active').removeClass('is-active');
          panel.find('.seat .search-item__data').text(classes[Status.search.seat]);
          var seat = $('[data-target="js-seat-value"] a.open-selector__item');
          for (var i=0; i < seat.length; i++){
            if (seat[i].dataset.id == Status.search.seat) {
              seat[i].className = 'open-selector__item js-select-item is-active';
              break;
            }
          }
        }
    ...
    methods.fetchSearch = function() {
      var status = {};

      var loader = function(query) {
        status.lock = true;
        var param = $.extend({}, Status.search);
        delete param.referer;
        var referer = Func.storage('referer');
        referer && (param.referer = referer);
        param.limit = 1000; // ページング
        if (Func.metafrom()) {
          Func.history('air/cond/cache', 'clear');
        }
        if ((document.referrer.match(/m\/air$/), Func.history('air/cond/cache').shift() === 'clear') ||
          (Func.history('tour/cond/cache').shift() === 'clear' ) ||
          (Func.metafrom() && Func.history('air/cond/cache').shift() === 'clear')) {
          param.cache = 'clear';
        }
        param._purchtype = Status.pathname[0];

        return $.ajax({
          method: 'POST',
          url: Func.url('/air/data/search', query || Func.storage(['session', 'code'])),
          contentType: 'application/json',
          data: JSON.stringify(param),
          dataType: 'json'
        })
        .fail(function(xhr, status, thrown) {
          if (xhr.responseJSON.message === 'parameterMismatch' && Status.pathname[0] === 'air') {
            Func.error('条件を再指定してください。', true);
            $('#loading').remove();
            var $accordionWrapper = $('.research-panel__header.current-search-info.js-toggle-accordion').closest('.js-accordion-wrapper');
          ...
          status.retry && clearInterval(status.retry);
          delete status.retry;
          Func.error(thrown.message, true);
        });
      };

      var sortTo = Entry.sort(function() {
        return status.entries;
      });
      var filterTo = Entry.filter(function() {
        return status.entries;
      });
      ...
      var maxContinued = 10;
      var key = null;
      var surchargeViewFlg = false;
      var execute = function(result) {
        if (result.result) {
          Entry.stack(result.result, status.entries, status.index);
          sortTo(result.result);
          filterTo(result.result);
        }
        if (document.referrer.match(/m\/air$/), Func.history('air/cond/cache').shift() === 'clear') {
          Func.history('air/cond/cache', false);
        }
        if (Func.history('tour/cond/cache').shift() === 'clear' ) {
          Func.history('tour/cond/cache', false);
        }
        result.key && (key = result.key);
        result.key && (key_ = result.key);
        if (result.status === 'loading' || result.status === 'continue') {
          if (!status.retry && key !== null) {
            var option = Func.storage(['session', 'code']);
            option.key = key;
            status.retry = setInterval(function() {
              status.lock || loader(option).done(execute);
            }, SETTINGS.AIR.DELAY);
          }
        }
        if (result.status !== 'loading') {
          if (status.before !== 'continue' || status.before !== 'loaded') {
            if (result.result){
              if(result.result.StatusInfo && result.result.StatusInfo.freeText && surchargeViewFlg === false){
                $('.research-panel').before(function() {
                  surchargeViewFlg = true;
                  return $('#js-modal-detail .caution-message').clone(true)
                    .find('h4').text('サーチャージに関して').end()
                    .find('p').html(result.result.StatusInfo.freeText.replace(/\r?\n/g, "<br />")).end();
                });
              }
              Entry.view(status.entries, key);
              execed = true;
              continued = 0;
            }
            if (status.before == 'continue' && result.status == 'continue' && !result.result){
              continued++;
              if(continued > maxContinued){
                status.retry && clearInterval(status.retry);
                delete status.retry;
                if (execed === false){
                  Func.error('該当のデータが見つかりませんでした', true);
                  $('.list-count').each(function() {
                    var format = $(this).data('format');
                    if (!format) {
                      $(this).data('format', format = $(this).text());
                    }
                    $(this).text(format.replace(/−/, 0));
                  });
                }
                continued = 0;
              }
        ...
        if (result.status === 'loaded') {
          ...
          delete status.retry;
          continued = 0;
          Func.refreshover(result.expiration);
        }
      ...
      loader().done(function(result) {
        Func.storage(['session', 'code'], result);
        status.index = {};
        status.entries = [];
        execute(result);
      });
    ...
    methods.fetchFavorite = function() {
      var option = {
        method: 'POST',
        url: Func.url('/air/data/favorite', Func.storage(['session'])),
        contentType: 'application/json',
        dataType: 'json'
      }
      $.ajax(option)
        .done(function(result) {
          if (result.status === 'loaded') {
            ...
            Entry.stack(result.result, entries, {});
            sortTo(result.result);
            filterTo(result.result);
            key_ = result.result.FareGroups.responseKey;
            Entry.view(entries, key_);
          } else {
            Func.error('お気に入りはありません');
          }
        })
        .fail(function(xhr, status, thrown) {
          Func.error(thrown, true);
        });
    };
    methods.metaSearchBind = function () {
      var changeFlg = Func.history('metasearch/paramchange').shift();
      if (Func.metafrom()) {
        if (Func.metaitemcodecheck()) {
          $('#selected-info').show();
        }
        if (spBackFlg == false) {
          Func.clear();
        ...
        Func.firstparamsave();
        var metaParam = {};
        metaParam['flight.1'] = Status.search['flight.1'];
        metaParam['flight.2'] = Status.search['flight.2'];
        metaParam['stay'] = Status.search['stay'];
        metaParam['traveler'] = Status.search['traveler'];
        if (Status.search['airline']) {
          metaParam['airline'] = Status.search['airline'];
        }
        if (Status.search['seat']) {
          metaParam['seat'] = Status.search['seat'];
        }
        var flightFrom =  qFlight.decode(Status.search['flight.1']);
        if (Status.pathname[0] === 'tour') {
          if (Status.search['room']) {
            metaParam['room'] = Status.search['room'];
          }
          if (Status.search['hotel']) {
            metaParam['hotel'] = Status.search['hotel'];
          }
          if (Status.search['region']) {
            metaParam['region'] = Status.search['region'];
          }
          Func.history('tour/cond', metaParam);
        } else {
          Func.history('air/cond', metaParam);
        }
        var metaDepartureParam = [];
        if (Status.search['flight.1']) {
          var flight1 = flightFrom;
          metaDepartureParam.push(flight1.departure);
        }
        Func.history('air/search/departure', metaDepartureParam);
        var metaArrivalParam = [];
        if (Status.search['flight.2']) {
          var flight2 = qFlight.decode(Status.search['flight.2']);
          metaArrivalParam.push(flight2.departure);
        }
        Func.history('air/search/arrival', metaArrivalParam);
        if (!Func.paramcheck(Status.pathname[0])) {
          var $accordionWrapper = $('.research-panel__header.current-search-info.js-toggle-accordion').closest('.js-accordion-wrapper');
          var $accordionToggle = $accordionWrapper.find('.js-toggle-accordion');
          var $accordionContents = $accordionWrapper.children('.js-accordion-contents');
          $accordionWrapper.addClass('is-open');
      ...
      } else if ((Func.metaitemcodecheck() && Status.pathname[0] == 'tour' && Status.pathname[1] == 'search') && !changeFlg ) {
        $('#selected-info').show();
      }
    };
    methods.missingParam = function () {
      $('#loading').remove();
      $('.l-section .section-header').hide();
      var build = Func.normalparam();
      if (build['room'] && build['room'].match(/^(\d+)-0?$/)) {
        var room = build['room'].split(/-/);
        build['room'] = room[0];
      }
      if (build['flight.1'] && !build['flight.2']) {
        var flightFrom = qFlight.decode(build['flight.1']);
        build['flight.2'] = Func.vsprintf('%s-%s %s%02d%02d', [flightFrom.arrival,flightFrom.departure,flightFrom.date.year, flightFrom.date.month, flightFrom.date.day]);
      }
      if (!build['flight.1'] && build['flight.2']) {
        var flightTo = qFlight.decode(build['flight.2']);
        build['flight.1'] = Func.vsprintf('%s-%s %s%02d%02d', [flightTo.arrival,flightTo.departure,flightTo.date.year, flightTo.date.month, flightTo.date.day]);
      }
      if (build['flight.1'] && build['flight.2'] && !build['stay']) {
        var flightFrom = qFlight.decode(build['flight.1']);
        var flightTo = qFlight.decode(build['flight.2']);
        build['stay'] = Func.vsprintf('%s%02d%02d-%s%02d%02d', [flightFrom.date.year, flightFrom.date.month, flightFrom.date.day,flightTo.date.year, flightTo.date.month, flightTo.date.day]);
      }
      //  jsのエラー対策
      if (!build['traveler']) {
        build['traveler'] = '1-0-0';
      }
      Func.history('air/cond', build);
      var metaDepartureParam = [];
      if (build['flight.1']) {
        var flight1 = qFlight.decode(build['flight.1']);
        metaDepartureParam.push(flight1.departure);
      }
      Func.history('air/search/departure', metaDepartureParam);
      var metaArrivalParam = [];
      if (build['flight.2']) {
        var flight2 = qFlight.decode(build['flight.2']);
        metaArrivalParam.push(flight2.departure);
      }
      Func.history('air/search/arrival', metaArrivalParam);
      var $accordionWrapper = $('.research-panel__header.current-search-info.js-toggle-accordion').closest('.js-accordion-wrapper');
      ...
      view: function(entries) {
        ...
        var roomembed = function(item, data, detail, sessionKey) {
            ...
            item.find('.selecthotel').off('click').on('click', function() {
                ...
                //Func.history('hotel/detail', JSON.stringify({hotelid: detail.hotelid, paymentmethods:data.PaymentMethods, price:hotelprice, roomTypeCode: data.roomTypeCode, rateCode: data.rateCode, rateKey: data.rateKey, time:new Date().getTime()}));
              ...
              for (var rIdx=0; rIdx < room.length; rIdx++){
                ...
                if (0 < room[rIdx].children.length){
                  for (var cIdx=0; cIdx < room[rIdx].children.length; cIdx++){
                    // 子供の年齢
                    param['chdA_' + rIdx + '_' + cIdx] = room[rIdx].children[cIdx];
                  }
                }
              }
              if (Status.search.source && Status.search.source.length > 0) {
            ...
            item.find('.button-area a.is-set').off('click').on('click', function() {
              ...
              $.ajax(option).done(function(result) {
                ...
                var href = Func.url('tour/hotel', Status.search, function(build) {
                    ...
                    //}
                    //var arrivalData = {fid:''};
                    //if (typeof airport.fid != 'undefined'){
                    //  arrivalData = airport;
                    //}
                    //else {
                    //  arrivalData = $('.search-airport .suggest-airport .suggested-city .suggested-city__item:contains('+airport.name+')').data();
                    //}
...
//            if (null == detailModalBase){
//              detailModalBase = $('#js-modal-detail').clone(true);
//            }
//            var modal = detailModalBase.clone(true);
//            modal.data('entry', data);
        ...
        var create = function() {
          if (entries.length > created.length) {
            ...
            var hotelDetail = Func.history('hotel/detail').shift();
            if (hotelDetail && Status.pathname[0] === 'tour' ) {
              hotelDetail = JSON.parse(hotelDetail);
              selectedHotelid = hotelDetail.hotelid;
            }
            $.each(entries, function() {
              if (!this._hidden && created.indexOf(this.hotelid) < 0) {
                if (-- pager >= 0) {
                  ...
                  var cItemObj = itemObj.clone(true);
                  if (!Status.search.hotel || Status.search.hotel != this.hotelid) {
                    cItemObj.removeClass('selected');
                    cItemObj.find('.selected-head').remove();
                    list.append(embed(cItemObj, this));
                  }
                ...
                if ( selectedHotelid ) {
                  if (this.hotelid == selectedHotelid) {
                    created.push(this.id);
                    var cItemObj = itemObj.clone(true);
                    metaSelected = 'Y';
                    $('#selected-info').hide();
                    list.prepend(embed(cItemObj, this));
                  } else if (Status.search.hotel && Status.search.hotel == this.hotelid && !metaSelected) {
                    cItemObj.removeClass('selected');
                    cItemObj.find('.selected-head').remove();
                    list.append(embed(cItemObj, this));
                  }
                } else {
                  if (Status.search.hotel && Status.search.hotel == this.hotelid && !metaSelected) {
                    created.push(this.id);
                    var cItemObj = itemObj.clone(true);
                    metaSelected = this.selected;
                    $('#selected-info').hide();
                    list.prepend(embed(cItemObj, this));
                  }
                }
            ...
            if (Func.metafrom() && !metaSelected) {
              $('#selected-info .selected-text').html('<span class="selected-notfind">大変申し訳ございません。</span>お選びいただいた日程で、お探しの条件に合う利用可能なホテルが見つかりませんでした。');
            }
          }
        };
      ...
      $('.search-airport').each(function() {
        ...
        var click = function() {
          ...
          var select = function(data) {
            ...
            if (((true == (0 < $('.search-airport.is-arrival .search-item__value').size() && 0 < $('.search-airport.is-arrival .suggest-airport .suggested-city').size())) || hotelId ) && !Func.metafrom() ){
              // DP用
      ...
      $(targets.join(',')).bind('click', function() {
        ...
        var searchHotel = function(arrivalID, dom) {
            ...
            if (true == $(dom).hasClass('is-set') || -1 < location.pathname.indexOf('/m/tour/')){
              ...
              // DPへ移動するのでフライト条件を滞在地からデフォルト設定する
//              var e = getLocation(arrivalID);
//              var arrivalName = e.cityname || e.airport;
//              var arrivalData = $('.search-airport .suggest-airport .suggested-city .suggested-city__item:contains('+airport.name+')').data();
            ...
            if (undefined == arrivalID || '' == arrivalID || null == arrivalID){
              if (Func.metafrom() && Status.search.hotel) {
                  conditions['hotel'] = Status.search.hotel;
                  conditions['source'] = Status.search.source;
              }
              $.extend(conditions, Func.metaparampullout());
              location.href = Func.url(locationurl, conditions, function(build) {
                var _condition = Func.history(historyPath).shift();
...
//                }
              ...
              location.href = Func.url(locationurl, conditions, function(build) {
                ...
                if (_condition != undefined){
                  ...
                  if (!conditions.region && !conditions.hotel && Status.pathname[0] === 'tour'){
                    build.region= result['found'][0]['id'];
                  }
                  if ($('[data-target="js-airline-value"] a.is-active').data('id')) {
                    build.airline = $('[data-target="js-airline-value"] a.is-active').data('id');
                  } else {
                    delete build.airline;
                  }
                  if ($('[data-target="js-seat-value"] a.is-active').data('id')) {
                    build.seat = $('[data-target="js-seat-value"] a.is-active').data('id');
                  } else {
                    delete build.seat;
                  }
                  // 航空券選択キャッシュ削除判定
                  ...
                  // 航空券選択キャッシュ削除判定
                  if (true == ('seat' in build) && false == (true == ('seat' in _condition) && true == ('seat' in build) && build['seat'] == _condition['seat']) || (_condition.seat && !build.seat) ) {
                    Func.history('tour/detail/airdom', false);
                    Func.history('air/detail', false);
                  }
                  // 航空券選択キャッシュ削除判定
                  if (true == ('airline' in build) && false == (true == ('airline' in _condition) && true == ('airline' in build) && build['airline'] == _condition['airline']) || (_condition.airline && !build.airline) ) {
                    Func.history('tour/detail/airdom', false);
                    Func.history('air/detail', false);
                  }
                  // 航空券選択キャッシュ削除判定
        ...
        //if (airport.fid) {
          // XXX 必ず3レーター -> regionと言う変換をする仕様の場合は以下
        //  arrivalID = airport.fid;
        //} else if (airport.id) {
          //conditions[airport.datatype] = airport.id;
        if (arrivalID == '' && airport.id) {
          //arrivalID = airport.id;
          //dataType = airport.datatype;
          conditions[airport.datatype] = airport.id;
          searchHotel('', this);
...
//              var option = {
//                method: 'GET',
//                url: Func.url('/hotel/data/suggestion', {q: flight.arrival, type:'airportcity' }),
//                contentType: 'application/json',
//                dataType: 'json'
//              };
//              $.ajax(option).done(function(result) {
                ...
                if (airport.datatype && airport.datatype === 'region'){
                  conditions['region'] = airport.id;
                }
                /*
          ...
          //          if (true !== (0 < $('.search-airport.is-arrival .search-item__value').size() && 0 < $('.search-airport.is-arrival .suggest-airport .suggested-city').size())){
//            return Func.error('*SEARCH_MISSING_STAYPLACE');
//          }
//          var arrivalID = $('.search-airport.is-arrival .search-item__value').text();
//          if (0 >= arrivalID.length){
//            return Func.error('*SEARCH_MISSING_STAYPLACE');
//          }
//          var option = {
//            method: 'GET',
...
//                airport = $('.search-form .search-hotel .search-item').data();
//            }
//            var departureID = $('.search-airport:not(.is-arrival) .search-item__value').text();
//            var flight1 = qFlight.decode(Status.search['flight.1']);
//            var flight2 = qFlight.decode(Status.search['flight.2']);
//            flight1.arrival = arrivalID;
//            flight1.departure = departureID;
//            flight1.date = airrange.from;
//            flight2.departure = arrivalID;
//            flight2.arrival = departureID;
//            flight2.date = airrange.to;
//            conditions['flight.1'] = qFlight.encode(flight1);
//            conditions['flight.2'] = qFlight.encode(flight2);
//            var traveler = {
//              adult: $('section:not(.modal) .search-form .js-input-adult').text()|0,
//              child: $('section:not(.modal) .search-form .js-input-child').text()|0,
//              baby: $('section:not(.modal) .search-form .js-input-baby').text()|0,
...
//              traveler.adult = 1;
//            }
//            if (traveler.adult + traveler.child + traveler.baby > 9) {
//              return Func.error('*SEARCH_TOO_MANY_TRAVELERS');
//            }
//            if (traveler.adult < traveler.baby) {
//              return Func.error('*SEARCH_TOO_MANY_BABIES');
...
//          });
    ...
    methods.restoreConditions = function(type, targetpath) {
      ...
      /*
      if (condition.region || condition.hotel) {
        var type = '';
        if (condition.region) {
            type = 'region';
        }
        ...
        var option = {
          method: 'GET',
          url: Func.url('/hotel/data/suggestion', {q: condition.region || condition.hotel, type:type}),
          contentType: 'application/json',
          dataType: 'json'
        };
        $.ajax(option).done(function(result) {
          if (result.found.length > 0) {
            var data = result.found.pop();
            if (targetpath == 'hotel'){
              var arrival = null;
              if (typeof data.fid != 'undefined'){
                arrival = getLocation(data.fid);
              }
              if (arrival) {
                $('.search-form .search-airport .search-item').data(data)
                .find('.label').text(arrival.cityname || arrival.airport);
              }
              else {
                $('.search-form .search-airport .search-item').data(data)
                .find('.label').text(data.name);
              }
            }
            else if (0 < $('.search-airport.is-arrival .suggest-airport .suggested-city').size() && true == ('fid' in data) && typeof data.fid == 'string'){
              $('.search-airport.is-arrival .search-item__value').text(data.fid);
            }
            else if (0 < $('.search-airport.is-arrival .suggest-airport .suggested-city .suggested-city__item:contains('+data.name+')').size()){
              // DPの場合
              var arrivalData = $('.search-airport.is-arrival .suggest-airport .suggested-city .suggested-city__item:contains('+data.name+')').data();
              if (undefined != arrivalData && arrivalData.id){
                $('.search-airport.is-arrival .search-item__value').text(arrivalData.id);
              }
            }
          }
        });
      ...
      if (condition.room) {
        var traveler = null;
        if (undefined != condition.traveler){
          traveler = qTraveler.decode(condition.traveler);
        }
        var room = qRoom.decode(condition.room);
        var children = [];
        var modal = $('#js-modal-people');
        modal.find('.js-step-rooms').val(0).trigger('click').val(1).trigger('click');
        $.each(room, function(index) {
          index > 0 && modal.find('.js-step-rooms').siblings('.is-plus').trigger('click');
          var block = modal.find('.js-people-selector').eq(index);
          var dist = function(self, n) {
            var d = n - self.val()|0;
          ...
          if (undefined == this.adult){
            dist(block.find('.js-people-num-adult'), this);
          }
          else {
            dist(block.find('.js-people-num-adult'), this.adult);
            dist(block.find('.js-view-age'), this.children.length);
            if (undefined != this.children) {
              $.each(this.children, function(index) {
                children.push(this);
                block.find('.js-age-item').eq(index).find('select').val([this]);
                block.find('.js-age-item').eq(index).find('select').trigger('change');
                if (null != traveler){
                  // DPの場合
                  var item = block.find('.js-age-item').eq(index);
                  if (item.find('.js-switch-seat').hasClass('is-checked-disable')) {
                    item.find('.js-switch-seat').prop('checked', true);
                  } else {
                    var useseat = traveler.children[children.length-1];
                    if (useseat != undefined) {
                      item.find('.js-switch-seat').prop('checked', useseat.seat);
                    }
                  }
                  setTimeout(function() {
                    item.find('.js-switch-seat').trigger('change').end();
      ...
      if (targetpath == 'hotel'){
        if (condition.stay) {
          Calendar.setValue(0, qStay.decode(condition.stay));
        }

        if ($('.research-panel .current-search-info__area') && 0 >= $('.research-panel .current-search-info__area').text().length){
          $('.research-panel .current-search-info__area').text(' ');
          var _data = {name:''};
          if (('region' in condition)) {
            _data.datatype = 'region';
            _data.id = condition.region;
          }
          else if (('hotel' in condition)) {
            _data.datatype = 'hotel';
            _data.id = condition.hotel;
          }
          if (!_data.id && Status.pathname[1] === undefined) {
            return true;
          }
          $('.search-form .search-hotel .search-item').data(_data).find('.label').text(' ').end();
          //var query = {q: data.ConditionsOfUse.Address.county, type:'type3', lat: data.ConditionsOfUse.Address.latitude, lon:data.ConditionsOfUse.Address.longitude };
          var query = {q: _data.id, type:_data.datatype };
          ...
          if (Func.metafrom() && Status.search.hotel) {
            ...
            var option = {
              method: 'POST',
              url: Func.url('/hotel/data/suggestion/area', Func.storage(['session', 'code'])),
              contentType: 'application/json',
              data: postdata,
              dataType: 'json'
            };
          ...
          $.ajax(option).done(function (result) {
            if (Status.search.region && result) {
              for (var didx = 0; didx < result.found.length; didx++) {
                if (Status.search.region == result.found[didx].id) {
                  _data = result.found[didx];
                  break;
                }
              }
            } else if(result){
              _data = result.found[0];
            }
            if (Func.metafrom() && Status.search.hotel && _data) {
              var _condition = Func.history('hotel/cond').shift();
              _condition.region = _data.id;
              Func.history('hotel/cond', _condition);
            }
            if (_data) {
              $('.research-panel .current-search-info__area').text(_data.name);
              $('.search-form .search-hotel .search-item').data(_data).find('.label').text(_data.name).end();
            }
          });
        }
      }
...
//        var option = {
//          method: 'GET',
//          url: Func.url('/hotel/data/suggestion', {q: Status.search.region || Status.search.hotel, type:type}),
//          contentType: 'application/json',
//          dataType: 'json'
//        };
//        $.ajax(option).done(function(result) {
//          if (result.found.length > 0) {
//            var data = result.found.pop();
//            if (true == panel.find('.current-search-info__area').hasClass('withhotel')){
//              //panel.find('.current-search-info__area').text(panel.find('.current-search-info__area').text()+' (' + data.name+'滞在)');
//            } else {
//              var arrival = null;
//              if (typeof data.fid != 'undefined'){
//                arrival = getLocation(data.fid);
//              }
//              if (arrival) {
//                panel.find('.current-search-info__area').text(arrival.cityname || arrival.airport);
//              }
//              else {
//                panel.find('.current-search-info__area').text(data.name);
//              }
//            }
//          }
//          defer.resolve();
//        });
      ...
      if (Status.search.stay && Status.search.stay.match(/^\d{8}-\d{8}$/)) {
        var stay = qStay.decode(Status.search.stay);
        if (stay !== null) {
    ...
    methods.fetchSearch = function() {
      ...
      var loader = function(query) {
        ...
        referer && (param.referer = referer);
        param.limit = 1000; // ページング
        if (true == (0 < $('.search-airport.is-arrival').size() && 0 < $('.search-airport.is-arrival .suggest-airport .suggested-city').size())){
          // DPの場合、パッケージ価格の計算をする
          param.PackageRate = 'Y';
        }
        if (Func.history('hotel/cond/cache').shift() === 'clear' ) {
          param.cache = 'clear';
        }
        // 日付が正当かチェック
        var chk_rangefrom = new Date(param.stay.substr(0,4), param.stay.substr(4,2) -1, param.stay.substr(6,2));
        var chk_nowdate = new Date();
        chk_rangefrom.setDate(chk_rangefrom.getDate() + 1);
        if (false === (chk_nowdate < chk_rangefrom)){
          // 過去日程の検索条件はエラー
        ...
        return $.ajax({
          method: 'POST',
          url: Func.url('/hotel/data/search', query || Func.storage(['session', 'code'])),
          contentType: 'application/json',
          data: JSON.stringify(param),
          dataType: 'json'
        })
      ...
      var execute = function(result) {
        ...
        if (Func.history('hotel/cond/cache').shift() === 'clear' ) {
          Func.history('hotel/cond/cache', false);
        }
        result.key && (key = result.key);
        result.key && (key_ = result.key);
        if (result.status === 'loading' || result.status === 'continue') {
          if (!status.retry && key !== null) {
            ...
            status.retry = setInterval(function() {
              status.lock || loader(option).done(execute);
            }, SETTINGS.HOTEL.DELAY);
          }
        }
        if (result.status !== 'loading') {
          if (status.before !== 'continue' || status.before !== 'loaded') {
            if (result.result){
              Entry.view(status.entries);
              continued = 0;
              execed = true;
            }
            if (status.before == 'continue' && result.status == 'continue' && !result.result){
    ...
    methods.fetchFavorite = function() {
      var option = {
        method: 'POST',
        url: Func.url('/hotel/data/favorite', Func.storage(['session'])),
        contentType: 'application/json',
        dataType: 'json'
      }
      $.ajax(option)
        .done(function(result) {
          if (result.status === 'loaded') {
            ...
            sortTo(result.result);
            filterTo(result.result);
            key_ = result.result.HotelSummarys.responseKey;
            Entry.view(entries, key_);
          } else {
            Func.error('お気に入りはありません');
          }
    ...
    methods.metaSearchBind = function() {
      if (Func.metafrom()) {
        ...
        var metaParam = {};
        metaParam['room'] = Status.search['room'];
        if (metaParam['room'] && metaParam['room'].match(/^(\d+)-0?$/)) {
          var room = metaParam['room'].split(/-/);
          metaParam['room'] = room[0];
        }
        metaParam['stay'] = Status.search['stay'];
        if (Status.search['source']) {
          metaParam['source'] = Status.search['source'];
        }
        if (Status.search['hotel']) {
          metaParam['hotel'] = Status.search['hotel'];
        } else if (Status.search['region']) {
          metaParam['region'] = Status.search['region'];
        }
        Func.history('hotel/cond', metaParam);
        if (!Func.paramcheck(Status.pathname[0])) {
          var $accordionWrapper = $('.research-panel__header.current-search-info.js-toggle-accordion').closest('.js-accordion-wrapper');
          var $accordionToggle = $accordionWrapper.find('.js-toggle-accordion');
          var $accordionContents = $accordionWrapper.children('.js-accordion-contents');
      ...
      } else if (Func.metaitemcodecheck() && Status.pathname[0] == 'tour' && Status.pathname[1] == 'hotel') {
        $('#selected-info').show();
      }
    };
    methods.missingParam = function () {
      var getWeekAfter = function() {
        var now = new Date();
        var nowms = now.getTime();
        var after = 14;
        after = after * 24 * 60 * 60 * 1000;
        var future = new Date(nowms + after);
      ...
      if (build['stay'] && !build['stay'].match(/^\d{8}-\d{8}$/)) {
        if (build['stay'].match(/^\d{8}$/)) {
          build['stay'] = build['stay'] + '-' + build['stay'];
        } else {
          var stay = getWeekAfter();
  ...
  var Tour = (function() {
    ...
    methods.missingParam = function () {
      ...
      var build = Func.normalparam();
      if (build['room']) {
        if (typeof build['room'] === 'string' && build['room'].match(/^(\d+)-0?$/)) {
          var room = build['room'].split(/-/);
          build['room'] = room[0];
        }
      }
  ...
  //Favorite.updateStatus();
  Func.brandini();

  if (Status.pathname[0] === 'air') {
    if (Func.metafrom() && !Func.paramcheck(Status.pathname[0]) && Func.metavalidity(Status.pathname) ) {
      Air.missingParam();
      Air.bindAirline();
      Air.bindAirport();
      Air.bindSearch(Status.pathname[1]);
      Calendar.bindUpdate();
      Air.restoreConditions(Status.pathname[1]);
      Air.embedConditions();
    } else {
      if (Status.pathname[1] === undefined) {
        Func.history('metasearch/param', false);
        Func.history('metasearch/paramchange', false);
      }
      if (
        Status.pathname[1] === undefined ||
        Status.pathname[1] === 'oneway' ||
        Status.pathname[1] === 'multi-city' ||
        Status.pathname[1] === 'abroad' ||
        Status.pathname[1] === 'search'
      ) {
        Air.metaSearchBind();
        Air.bindAirline();
        Air.bindAirport();
      ...
      if (
        Status.pathname[1] === 'search' ||
        Status.pathname[1] === 'favorite'
      ) {
        Favorite.bindSwitch('air');
      }
    }
  }

  if (Status.pathname[0] === 'hotel') {
    if (Func.metafrom() && !Func.paramcheck(Status.pathname[0]) && Func.metavalidity(Status.pathname) ) {
      Hotel.missingParam();
      Hotel.bindAirport();
      Hotel.bindSearch(Status.pathname[1]);
      Calendar.bindUpdate();
      Hotel.restoreConditions(Status.pathname[1]);
  ...
  if (Status.pathname[0] === 'tour') {
    if (Func.metafrom() && !Func.paramcheck(Status.pathname[0]) && Func.metavalidity(Status.pathname) ) {
      ...
      Air.bindAirline();
      Air.bindAirport();
      Tour.bindSearch(Status.pathname[1]);
      Calendar.bindUpdate();
      Air.restoreConditions(Status.pathname[1], 'tour');
      Air.embedConditions();
    } else {
      if (Status.pathname[1] === undefined) {
        Func.history('metasearch/param', false);
      ...
      if (
        ...
        Calendar.bindUpdate();
        Air.restoreConditions(Status.pathname[1], 'tour');
        Hotel.restoreConditions(Status.pathname[1], 'tour');
      }
      if (Status.pathname[1] === 'search') {
        Air.embedConditions();
        Hotel.embedConditions();
